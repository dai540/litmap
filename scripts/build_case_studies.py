...
def update_tutorial_index(studies: list[StudySpec]) -> None:
    cards = "\n".join(
        (
            f'<article class="card"><div class="eyebrow">Case study</div>'
            f'<h3><a href="{study.slug}.html">{study.title}</a></h3>'
            f'<p>{study.purpose[0]}</p></article>'
        )
        for study in studies
    )
    html_doc = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>litmap tutorials</title><link rel="icon" type="image/svg+xml" href="../assets/litmap-icon.svg">
<style>:root{{--bg:#f7f5ef;--panel:#ffffff;--ink:#182128;--muted:#5d6770;--line:#e6e8eb;--accent:#0f6c5c;}}*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--ink);font-family:"Segoe UI","Helvetica Neue",Arial,sans-serif;line-height:1.65}}a{{color:var(--accent);text-decoration:none}}a:hover{{text-decoration:underline}}.page{{max-width:1180px;margin:0 auto;padding:28px 24px 56px}}.hero,.panel,.card{{background:var(--panel);border:1px solid var(--line);border-radius:18px;box-shadow:0 10px 28px rgba(23,32,41,0.05)}}.hero{{padding:24px;margin-bottom:24px;display:grid;grid-template-columns:1.3fr 0.9fr;gap:24px}}.hero-head{{display:flex;gap:16px;align-items:center;margin-bottom:14px}}.hero-head img{{width:62px;height:62px;border-radius:18px}}.hero h1,.panel h2,.card h3{{margin-top:0}}.eyebrow{{color:var(--accent);font-size:0.82rem;text-transform:uppercase;letter-spacing:0.08em;font-weight:700;margin-bottom:8px}}.small{{color:var(--muted);font-size:0.94rem}}.panel{{padding:20px}}.grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;margin-bottom:24px}}.card-grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}}.card{{padding:18px}}.card p{{margin:0;color:var(--muted);font-size:0.95rem}}.list{{list-style:none;margin:0;padding:0}}.list li{{border-top:1px solid var(--line);padding:10px 0}}.list li:first-child{{border-top:0;padding-top:0}}@media (max-width:920px){{.hero,.grid,.card-grid{{grid-template-columns:1fr}}.page{{padding:18px 14px 40px}}}}</style></head>
<body><main class="page"><section class="hero"><section><div class="eyebrow">Tutorials</div><div class="hero-head"><img src="../assets/litmap-icon.svg" alt="litmap icon"><h1>Case-study articles</h1></div><p>These articles are the fastest way to understand what <code>litmap</code> does in practice. Each one starts with a domain question, runs a deterministic clustering workflow, and ends with artifacts a review team could actually inspect.</p><p class="small">Every article includes a result map, cluster tables, PCA diagnostics, reading-order guidance, and review-oriented follow-up sections.</p></section><aside class="panel"><h2>On this page</h2><ul class="list"><li><a href="../index.html">Docs home</a><div class="small">Back to the main project entry point.</div></li><li><a href="../getting-started.md">Getting started</a><div class="small">Install and run the package.</div></li><li><a href="../api.md">API surface</a><div class="small">See the stable Python interface.</div></li><li><a href="https://github.com/dai540/litmap">GitHub repository</a><div class="small">Browse source and development history.</div></li></ul></aside></section><section class="grid"><section class="panel"><div class="eyebrow">What you get</div><h2>Shared article structure</h2><ul class="list"><li><strong>Headline findings</strong><div class="small">A quick read of the main structural results.</div></li><li><strong>What to read first</strong><div class="small">Representative clusters and suggested starting papers.</div></li><li><strong>PCA summary</strong><div class="small">Component tables that explain why the map looks the way it does.</div></li><li><strong>Decision memo</strong><div class="small">A review-style interpretation of what the run implies.</div></li></ul></section><section class="panel"><div class="eyebrow">Outputs</div><h2>Artifacts behind each article</h2><ul class="list"><li><code>labels.csv</code><div class="small">Paper-level cluster assignments and probabilities.</div></li><li><code>cluster_summary.csv</code><div class="small">Cluster size, persistence, and theme summary.</div></li><li><code>coords_2d.csv</code><div class="small">Display-space coordinates used in the map.</div></li><li><code>map_interactive.html</code><div class="small">Interactive Plotly output linked from each article.</div></li></ul></section></section><section class="card-grid">{cards}</section></main></body></html>"""
